...
namespace AuthCore.Infrastructure.Security
{
    ...
    public sealed class JwtTokenProvider : IJwtTokenProvider
    {
            ...
            ILogger<JwtTokenProvider> logger)
        {
            ...
            _logger = logger;
            ECDsa ecdsaPrivateKey = CreatePrivateKey(_ecdsa.PrivateKey);
        ...
        /// <summary>Operação para importar chave privada ECDSA nos formatos PKCS#8 ou SEC1.</summary>
        /// <param name="privateKey">Chave privada em DER.</param>
        private static ECDsa CreatePrivateKey(byte[] privateKey)
        {
            ECDsa ecdsaPrivateKey = ECDsa.Create();
            try
            {
                ecdsaPrivateKey.ImportPkcs8PrivateKey(privateKey, out _);
                return ecdsaPrivateKey;
            }
            catch (CryptographicException)
            {
                ecdsaPrivateKey.ImportECPrivateKey(privateKey, out _);
                return ecdsaPrivateKey;
            }
        }
