...
namespace AuthCore.Infrastructure.Security
{
    ...
    public sealed class SessionState : ISessionState
    {
        ...
        public async Task<User> GetCurrentUser()
        {
            ...
            {
                ClearCookies();
                await _sessionRepository.DeleteAsync(session.Id.Value);
                throw new UnauthorizedException();
            }
        ...
        public async Task<Session> GetCurrentSession()
        {
            ...
            Session session = await LoadAndValidateSessionAsync(hashedSession);
        ...
        /// <summary>Operação para carregar e validar a sessão do repositório.</summary>
        /// <param name="hashedSession">Hash da sessão extraída do cookie.</param>
        private async Task<Session> LoadAndValidateSessionAsync(string hashedSession)
        {
